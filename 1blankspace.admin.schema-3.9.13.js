...
{
	data: 	{
                lab: (window.location.host.indexOf('mydigitalstructure-lab.cloud') != -1),
                superUser: false,
                dataTypes:
                {
                    Numeric: 1,
	...
	show:		function (oParam, oResponse)
				{
					...
					{	
						...
						oSearch.addField('addavailable,advancedsearchavailable,endpoint,endpointtext,endpointother,endpointothertext,notes,object,objecttext,' +
										'removeavailable,title,name,unrestrictedaccess,unrestrictedloggedonaccess,updateavailable,useavailable,' +
                                        'status,statustext,limittype,limittypetext');
					...
					else
					{
						...
						$('#ns1blankspaceAdminSchemaMethodsExport').button(
						{
							label: "Export"
						})
						.click(function()
						{
							 ns1blankspace.admin.schema.export.methods.init();
						});

						$('#ns1blankspaceAdminSchemaMethodsImport').button(
						{
							label: "Import"
						})
						.click(function()
						{
							 ns1blankspace.admin.schema.import.methods.init();
						});
					}
				},	
					...
					if (ns1blankspace.user.super)
					{
						aHTML.push('<td style="width:70px;text-align:right;" class="ns1blankspaceRow">' + 
									...
									'<span id="ns1blankspaceAdminSchemaMethods_edit-' + oRow.id + '" class="ns1blankspaceMethodEdit"' +
                                        ...
                                        ' data-limittype="' + oRow["limittype"] + '"' +
										' data-limittypetext="' + oRow["limittypetext"] + '"' +
										' data-notes="' + window.btoa(oRow["notes"]) + '"' +
										'></span></td>');
					}
						...
						if (oDetail)
						{
							sHTML = '<table class="table table-condensed">' +
										'<tr><td style="width:40%;">ID</td><td>' + oDetail.id + '</td></tr>' +
                                            ...
                                            '<tr><td>Limiting</td><td>' + oDetail.limittypetext + '</td></tr>' +
	...
	edit:		function (oParam, oResponse)
				{
                    ...
                    aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'<tr class="ns1blankspace">' +
									'<td class="ns1blankspaceRadio">' +
									'<input type="radio" id="radioLimitType1" name="radioLimitType" value="1"/>As Per User/Space' +
									'<br /><input type="radio" id="radioLimitType2" name="radioLimitType" value="2"/>Sensitive' +
									'<br /><input type="radio" id="radioLimitType3" name="radioLimitType" value="3"/>None' +
									'</td></tr>');
					...
					if (iID != '')
					{
                        ...
                        $('[name="radioLimitType"][value="' + ns1blankspace.util.getData(oParam, 'data-limittype').value + '"]').attr('checked', true);
	...
	save: 	function (oParam, oResponse)
				{
					if (oResponse == undefined)
					{
						...
						var oData =
						{
                            ...
                            limittype: $('input[name="radioLimitType"]:checked').val(),
...
ns1blankspace.admin.schema.export =
{
	methods:
	{
		data: {},
		...
		init: function (oParam, oResponse)
		{
			...
			else
			{
				if (oResponse.data.rows.length != 0)
				{
					ns1blankspace.admin.schema.export.methods.data.all = oResponse.data.rows;
					ns1blankspace.admin.schema.export.methods.process(oParam);
				}
			}
		},
		...
		process: function (oParam, oResponse)
		{		
			ns1blankspace.admin.schema.export.methods.data.file = [];
			var aFile = ns1blankspace.admin.schema.export.methods.data.file;
			...
			$.each(ns1blankspace.admin.schema.export.methods.data.all, function (m, method)
			{
				aFileMethods.push('\t\t\t{"title": "' + method.title + '",' +
											' "name": "' + method.name + '",' +
											' "useavailable": "' + method.useavailable + '",' +
											' "addavailable": "' + method.addavailable + '",' +
											' "updateavailable": "' + method.updateavailable + '",' +
											' "removeavailable": "' + method.removeavailable + '",' +
											' "endpoint": "' + method.endpoint + '",' +
											' "endpointtext": "' + method.endpointtext + '",' +
											' "endpointother": "' + method.endpointother + '",' +
											' "endpointothertext": "' + method.endpointothertext + '",' +
											' "object": "' + method.object + '",' +
											' "objecttext": "' + method.objecttext + '",' +
											' "unrestrictedaccess": "' + method.unrestrictedaccess + '",' +
											' "unrestrictedloggedonaccess": "' + method.unrestrictedloggedonaccess + '",' +
											' "status": "' + method.status + '",' +
											' "statustext": "' + method.statustext + '",' +
											' "notes": "' + window.btoa(method.notes) + '"' +
											'}');
			});
			...
			{
				data: aFile.join('\n'),
				filename: 'mydigitalstructure-schema-methods.json'
			});
		}
	}
}

ns1blankspace.admin.schema.import =
{
	methods:
	{
		data: {},
		...
		init: function (oParam, oResponse)
		{
			$('#ns1blankspaceAdminSchemaMethodsColumn2').html('');
			$('#ns1blankspaceAdminSchemaMethodsColumn1').html(ns1blankspace.attachments.upload.show(
			{	
				object: 29,
				objectContext: -1,
				label: '',
					...
					$.ajax(
					{
						...
						success: function(data)
						{
							...
							{	
								...
								ns1blankspace.attachments.upload.submit({submit: true, functionPostUpdate: ns1blankspace.admin.schema.import.methods.process});
							}
						}
					});
				}
		...
		process: function (oParam)
		{
			$('#ns1blankspaceAdminSchemaMethodsColumn1').html('Processing...')
			...
			$.ajax(
			{
				...
				success: function(data)
				{
					var oFileData = JSON.parse(data.filedata);
					ns1blankspace.admin.schema.import.methods.data.all = oFileData.schema.methods;

					_.each(ns1blankspace.admin.schema.import.methods.data.all, function (method)
					{
						method.notes = window.atob(method.notes)
					});

					ns1blankspace.admin.schema.import.methods.existing();
				}
			});
		},

		existing: function (oParam, oResponse)
		{
			if (oResponse == undefined)
			{
				var oSearch = new AdvancedSearch();
			...
			else
			{
				if (oResponse.data.rows.length != 0)
				{
					ns1blankspace.admin.schema.import.methods.data.existing = oResponse.data.rows;
					ns1blankspace.admin.schema.import.methods.check(oParam);
				}
			}
		},
		...
		{
			var oExistingMethod;
			var bExistingUpToDate;

			ns1blankspace.admin.schema.import.methods.data.status =
			{
				uptodate: [],
				needsupdating: [],
				needscreating: []
			...
			_.each(ns1blankspace.admin.schema.import.methods.data.all, function (oImportMethod)
			{
				oExistingMethod = _.clone(_.find(ns1blankspace.admin.schema.import.methods.data.existing,
							function (existingMethod) {return existingMethod.title == oImportMethod.title}));

				if (oExistingMethod == undefined)
				{
					ns1blankspace.admin.schema.import.methods.data.status.needscreating.push(oImportMethod)
				}
				else
				{
					var iID = oExistingMethod.id;
					delete oExistingMethod.id;

					if (_.isEqual(oImportMethod, oExistingMethod))
					{
						oImportMethod.id = iID;
						ns1blankspace.admin.schema.import.methods.data.status.uptodate.push(oImportMethod)
					}
					else
					{
						oImportMethod.id = iID;
						ns1blankspace.admin.schema.import.methods.data.status.needsupdating.push(oImportMethod)
					}
				}
			});
			...
			aHTML.push('<table id="ns1blankspaceAdminSchemaMethodsImportNeedsCreating" class="ns1blankspace">' +
								'<tr class="ns1blankspaceHeaderCaption">' +
								'<td class="ns1blankspaceHeaderCaption">To Be Created</td>' +
								'</tr>');

			if (ns1blankspace.admin.schema.import.methods.data.status.needscreating.length == 0)
			{
				aHTML.push('<tr><td class="ns1blankspaceRow">No methods need creating.</td>');
			}
			else
			{
				_.each(ns1blankspace.admin.schema.import.methods.data.status.needscreating, function (method)
				{
					aHTML.push('<tr><td class="ns1blankspaceRow">' + method.title + '</td></tr>');
				})
			}

			aHTML.push('</table>')

			aHTML.push('<table id="ns1blankspaceAdminSchemaMethodsImportNeedsUpdating" class="ns1blankspace" style="margin-top:18px;">' +
								'<tr class="ns1blankspaceHeaderCaption">' +
								'<td class="ns1blankspaceHeaderCaption">To Be Updated</td>' +
								'</tr>');

			if (ns1blankspace.admin.schema.import.methods.data.status.needsupdating.length == 0)
			{
				aHTML.push('<tr><td class="ns1blankspaceRow">No methods need updating.</td>');
			}
			else
			{
				_.each(ns1blankspace.admin.schema.import.methods.data.status.needsupdating, function (method)
				{
					aHTML.push('<tr><td class="ns1blankspaceRow">' + method.title + '</td></tr>');
				})
			}
			...
			if (ns1blankspace.admin.schema.import.methods.data.status.needscreating.length != 0 ||
						ns1blankspace.admin.schema.import.methods.data.status.needsupdating.length != 0)
			{
				var aHTML = [];
				...
				aHTML.push('<tr><td style="padding-top:0px;">' +
								'<span id="ns1blankspaceAdminSchemaMethodsImportUpdateSchema" class="ns1blankspaceAction">Update Schema</span></td></tr>');

				aHTML.push('</table>');

				$('#ns1blankspaceAdminSchemaMethodsColumn2').html(aHTML.join(''))

				$('#ns1blankspaceAdminSchemaMethodsImportUpdateSchema').button(
				{
					label: "Update Schema"
				})
				.click(function()
				{
					 ns1blankspace.admin.schema.import.methods.updateSchema();
				});
			}
		},
		...
		{
			ns1blankspace.admin.schema.import.methods.data.status.update =
				_.concat(ns1blankspace.admin.schema.import.methods.data.status.needscreating,
								ns1blankspace.admin.schema.import.methods.data.status.needsupdating)

			_.each(ns1blankspace.admin.schema.import.methods.data.status.update, function (oData, d)
			{
				$('#ns1blankspaceAdminSchemaMethodsColumn1').html(oData.title + '...');
				...
				$.ajax(
				{
					...
					success: function(data)
					{
						if (d == ns1blankspace.admin.schema.import.methods.data.status.update.length -1)
						{
							ns1blankspace.status.message('Schema updated.')
							ns1blankspace.admin.schema.methods.init();
						}
					}
				});
