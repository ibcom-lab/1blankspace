...
ns1blankspace.util.financial.accounts =
{
	...
	types: function (oParam, oResponse)
	{
		...
		else
		{
            ...
            _.each(ns1blankspace.util.financial.accounts.data.defaults.typeRoots, function (typeRoot)
            {
                typeRoot.title = typeRoot.title.replace('\r\n', ' ');
            })
		...
		typeRoots: function (oParam, oResponse)
		{
			...
			else
			{
				...
				else
				{
					...
					_.each(ns1blankspace.util.financial.accounts.data.accounts.typeRoots, function (typeRoot)
					{
						typeRoot.title = typeRoot.title.replace('\r\n', ' ');
					})
