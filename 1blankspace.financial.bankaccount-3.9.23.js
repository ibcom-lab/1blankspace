	...
	layout: 	function ()
				{	
					...
					aHTML.push('<tr class="ns1blankspaceControl">' +
									...
									'<br /><div class="ns1blankspaceSubNote">with transactions from <span class="ns1blankspaceFinanialBankAccountBank">bank</span></div></td>' +
									'</tr>');
					...
					edit:	function (oParam, oResponse)
							{
								...
								if (iStep == 1)
								{
									...
									aHTML.push('<tr><td style="padding-top:5px;" id="ns1blankspaceItemAddSearchResults"><span class="ns1blankspaceSub" style="font-size:0.75em;">Press <span style="font-style:italic;">enter</span> to see all or<br />start typing part of the account title.</span></td></tr>');
								...
								process: function (oParam, oResponse)
											{
												...
												{	
													...
													{
														ns1blankspace.status.working('Creating... (' + (iIndex + 1) + '/' + ns1blankspace.financial.bankAccount.mapping.create.data.transactions.length + ')');
														
														var oTransaction = ns1blankspace.financial.bankAccount.mapping.create.data.transactions[iIndex];
														var bSave;

														if (ns1blankspace.option.financialNeedContact)
														{
															bSave = ((oTransaction.contactbusiness != '' || oTransaction.contactperson != '') && (oTransaction.financialaccount != ''))
														}
														else
														{
															bSave = (oTransaction.financialaccount != '')
														}
					...
					show:		function (oParam, oResponse)
								{
									if (oResponse == undefined)
									{
										...
										oSearch.addField('startdate,enddate,processeddate,reference,createdusertext,createddate,notes');
										oSearch.addFilter('bankaccount', 'EQUAL_TO', ns1blankspace.objectContext);
					...
					row:		function (oRow)
								{
									...
									if (oRow.processeddate != ''  && (oRow.startdate != '' || oRow.enddate != ''))
									{
										...
										if (oRow.enddate != '')
										{		
											aHTML.push('<div>' + oRow.enddate + '</div>');	
										}

										if (oRow.startdate != oRow.enddate)
										{		
											aHTML.push('<div> to ' + oRow.startdate + '</div>');	
										}

                                        if (oRow.notes != '')
										{		
											aHTML.push('<div style="font-size:0.75rem; margin-top:1px; color:lightgray;">' + oRow.notes.replace('{source:', '').replace('}', '') + '</div>');	
										}
									...
									show:		function(oParam, oResponse)
												{
													...
													else
													{
														...
														{	
															oParam = ns1blankspace.util.setParam(oParam, 'index', 0);
															oParam = ns1blankspace.util.setParam(oParam, 'status', 3);
															ns1blankspace.financial.bankAccount.mapping.create.init(oParam)
														}).
														css('width', '100px');
													...
													create: 
													{
														init: function (oParam)
														{
															...
															else
															{
																...
																{
																	bNeedInput = (oTransaction.financialaccount == '')
																}
															...
															else
															{
																...
																else
																{
																	...
																	oParam = $.extend(true, oParam,
																	{
																		...
																		taxType: oTransaction.taxtype,
																		itemDescription: oTransaction.description
																	});		// 3.8.4 cb: added taxType and itemDescription
														...
														show: function (oParam)
														{
															...
															if (sObject == 'journal')
															{
																...
																//-- Based on cassandra.buono@alt-designit.com.au Email 14FEB2025
																// aHTML.push('<div class="ns1blankspaceAction" id="ns1blankspaceBankAccountTransferSave">Save</div></div>');
																aHTML.push('<div class="ns1blankspaceAction" id="ns1blankspaceBankAccountTransferSave-' + oTransaction.id + '">Save</div></div>');
																...
																$('#ns1blankspaceBankAccountTransferSave-' + oTransaction.id).button(
																{
																	label: "Save"
																})
																.click(function() 
					...
					row:		function (oRow)
								{
									...
									else
									{
										...
										aHTML.push('<div style="margin-bottom:8px;" id="ns1blankspaceFinancialBankAccountReconcileCurrentBalance" class="ns1blankspaceSub">' +
									 					 '</div>');
										...
										aHTML.push(oRow.statementdate + '<div><span id="ns1blankspaceFinancialBankAccountReconcile_balance-' + oRow.id + '">$' +
									 					oRow.statementbalance + '</div>' +
									 						'<div class="ns1blankspaceSub">' +
															'statement balance</div>');
									}						
					...
					refresh:	function (oParam, oResponse)
								{
									...
									else
									{
										...
										var cPreviousBalance = (oResponse.PreviousBalance).parseCurrency();
										...
										ns1blankspace.financial.bankAccount.reconcile.data.locked =
										{
											debits: cPayments,
											credits: cReceipts,
											previousBalance: cPreviousBalance,
											currentBalance: cPreviousBalance + cReceipts - cPayments
										}
												
										$('#ns1blankspaceFinancialBankAccountReconcileCurrentBalance').html(
												'<div style="margin-bottom:1px;" class="ns1blankspaceSub">' +
												'$' + (ns1blankspace.financial.bankAccount.reconcile.data.locked.currentBalance).formatMoney(2, ".", ",")  + '</div>' +
												'<div style="margin-left: 5px; margin-bottom:7px; border-bottom-color: #9e9e9e6e;border-bottom-width: 1px;border-bottom-style: solid; padding-bottom: 4px;" class="ns1blankspaceSub">' +
												'current balance</div>');
										...
										{	
											aHTML.push('<div style="width: 15px; margin-right:2px; margin-bottom:10px; float:right;" class="ns1blankspaceBankAccountRecoRefresh" id="ns1blankspaceBankAccountRecoRefresh-' + iReconciliation + '">' +
															'</div>');

											aHTML.push('<div style="width: 15px; margin-right:2px; margin-bottom:10px; float:right;" class="ns1blankspaceBankAccountRecoEdit" id="ns1blankspaceBankAccountRecoEdit-' + iReconciliation + '">' +
															'</div>');
										}

										if (iMode == 1 && iStatus == 1 && parseFloat((oResponse.outofbalance).parseCurrency()) == 0)
										{	
											aHTML.push('<div style="width:15px; margin-right:2px; margin-bottom:10px; float:right;" class="ns1blankspaceBankAccountRecoLock" id="ns1blankspaceBankAccountRecoLock-' + iReconciliation + '">' +
															'</div>');
										}

										if (ns1blankspace.option.bootstrap)
										{
											aHTML.push('<div class="btn-group-vertical" role="group" data-toggle="buttons" style="width:105px; text-align:right; margin-left:5px; margin-right:3px; margin-bottom:16px;" id="ns1blankspaceBankAccountColumnItemType">');
											
											aHTML.push('<label class="btn btn-default btn-sm' + (iType==2?' active':'') + '" id="ns1blankspaceBankAccountColumnItemType-2-' + iReconciliation + '">' +
																'<input type="radio" name="radioType" data-1blankspace="ignore" ' + (iType==2?' checked="checked"':'') + '/>' +
																'<div>Incoming</div><div class="ns1blankspaceSubNote">(Credits)</div>' +
																'</label>');

											aHTML.push('<label class="btn btn-default btn-sm' + (iType==1?' active':'') + '" id="ns1blankspaceBankAccountColumnItemType-1-' + iReconciliation + '">' +
																'<input type="radio" name="radioType" data-1blankspace="ignore" ' + (iType==1?' checked="checked"':'') + '/>' +
																'<div>Outgoing</div><div class="ns1blankspaceSubNote">(Debits)</div>' +
																'</label>');
											...
											if (iMode == 1)
											{	
												aHTML.push('<div class="btn-group-vertical" role="group" data-toggle="buttons" style="width:105px; text-align:right; margin-left:5px; margin-right:3px; margin-bottom:5px;" id="ns1blankspaceBankAccountColumnItemMode">');

												aHTML.push('<label class="btn btn-default btn-sm active" id="ns1blankspaceBankAccountColumnItemMode-1-' + iReconciliation + '">' +
																'<input type="radio" name="radioMode" data-1blankspace="ignore" checked="checked" />' +
																'Unreconciled' +
																'</label>');

												aHTML.push('<label class="btn btn-default btn-sm" id="ns1blankspaceBankAccountColumnItemMode-2-' + iReconciliation + '">' +
																'<input type="radio" name="radioMode" data-1blankspace="ignore" />' +
																'Reconciled' +
																'</label>');

												aHTML.push('</div>');

												aHTML.push('<div style="font-size:0.875em; text-align:center; margin-top:10px; margin-bottom:4px; margin-left:2px;" id="ns1blankspaceControlContext_reco_summary_statement-' + iReconciliation + '"></div>');
											}
										}
										else
										{
											...
											{	
												...
												aHTML.push('<input style="width: 100%;" type="radio" id="ns1blankspaceBankAccountColumnItemMode-1-' + iReconciliation + '" name="radioMode" checked="checked" /><label for="ns1blankspaceBankAccountColumnItemMode-1-' + iReconciliation + '" style="width:90px; margin-bottom:1px; padding-left:1px; padding-right:2px;">' +
																'Unreconciled</label>');
												aHTML.push('<input style="width: 100%;" type="radio" id="ns1blankspaceBankAccountColumnItemMode-2-' + iReconciliation + '" name="radioMode" /><label for="ns1blankspaceBankAccountColumnItemMode-2-' + iReconciliation + '" style="width: 90px; padding-left:1px; padding-right:2px;">' +
																'Reconciled</label>');
												aHTML.push('</div>');

												aHTML.push('<div style="font-size:0.875em; text-align:center; margin-top:10px; margin-bottom:4px; margin-left:2px; padding:6px;" id="ns1blankspaceControlContext_reco_summary_statement-' + iReconciliation + '"></div>');
											}
										}
										...
										ns1blankspace.financial.bankAccount.reconcile.attachment.show(
										{
										...
										$('#ns1blankspaceBankAccountColumnItemMode ' + sSelect).click(function()
										{
											var aID = (this.id).split('-');
										...
										.click(function()
										{
											...
											$.ajax(
											{
												...
												success: function()
												{
													...
													ns1blankspace.financial.bankAccount.refresh();
												}
											});
										})
										.css('font-size', '0.925em');
						...
						attachment: 
								{
									...
									init: function (oParam)
									{
										ns1blankspace.financial.bankAccount.reconcile.attachment.data.objectContext = ns1blankspace.util.getParam(oParam, 'objectContext').value;
										ns1blankspace.financial.bankAccount.reconcile.attachment.upload()
									},

									upload: function (oParam)
									{
										var sHTML;
										var sBank = '<span class="ns1blankspaceBankAccountImportBank">' + ns1blankspace.objectContextData.bank + '</span>\'s website';
										if (sBank == '') {sBank = ' your banks website'}

										sHTML = '<div style="margin-top:2px; margin-bottom:10px; margin-left:1px; font-size:0.75em;" class="ns1blankspaceSub" id="ns1blankspaceBankReconcilationStatementDownload"></div>';

										sHTML = sHTML + ns1blankspace.attachments.upload.show(
										{	
											object: 158,
											objectContext: ns1blankspace.financial.bankAccount.reconcile.attachment.data.objectContext,
											label: 'Upload a statement (for reference) from ' + sBank  + '.',
											showUpload: false
										});

										sHTML = sHTML + '<div style="margin-top:10px; margin-left:3px;" class="ns1blankspaceSub" id="ns1blankspaceBankReconcilationStatementStatus"></div>';
										
										$('#ns1blankspaceBankAccountReconcileAttachmentContainer').html(sHTML);
											...
											if ($(this).val() != '')
											{
												...
												.click(function()
												{
													...
													else
													{
														...
														{
															submit: true,
															functionPostUpdate: ns1blankspace.financial.bankAccount.reconcile.attachment.process
														});
													}
												});
											}
									...
									process: function (oParam)
									{
										ns1blankspace.status.message('Uploaded')
										ns1blankspace.financial.bankAccount.reconcile.attachment.upload()
									},

									show: function (oParam, oResponse)
									{
										var sXHTMLElementID  = ns1blankspace.util.getParam(oParam, 'xhtmlElementID', {"default": 'ns1blankspaceBankReconcilationStatementDownload'}).value;
										var iObjectContext  = ns1blankspace.util.getParam(oParam, 'objectContext', {"default": ns1blankspace.financial.bankAccount.reconcile.attachment.data.objectContext}).value;
										var sDownloadText = ns1blankspace.util.getParam(oParam, 'downloadText').value;

										if (oResponse == undefined)
										{
											ns1blankspace.status.working('Getting statement...');

											var oSearch = new AdvancedSearch();
											oSearch.method = 'CORE_ATTACHMENT_SEARCH';
											oSearch.addField('filename,download');
											oSearch.addFilter('object', 'EQUAL_TO', '158');
											oSearch.addFilter('objectcontext', 'EQUAL_TO', iObjectContext);
											
											oSearch.rows = 1;
											oSearch.sort('id', 'desc')
											oSearch.getResults(function(data) {ns1blankspace.financial.bankAccount.reconcile.attachment.show(oParam, data)});
										}
										else
										{
											...
											{
												if (sDownloadText == undefined) {sDownloadText = oResponse.data.rows[0].filename}

												$('#' + sXHTMLElementID).html(
													'<a target="_self" href="' + oResponse.data.rows[0].download + '" class="ns1blankspaceNoUnloadWarn">' +  sDownloadText + '</a>');
											}
											else
											{
												$('#' + sXHTMLElementID).html('');
						...
						unlock:
								{
									...
									show: function (oParam, oResponse)
									{
										...
										$('#ns1blankspaceBankAccountColumnItemType').before(
											'<div style="margin-left:5px; margin-right:2px; margin-bottom:10px; float:right;" ' +
									...
									show:		function (oParam, oResponse)
												{
													...
													{	
														...
														oSearch.addFilter('bankaccount', 'EQUAL_TO', ns1blankspace.objectContext);
														//include 4 - previously transferred ??
                                                        //oSearch.addFilter('status', 'IN_LIST', '3,4');
														oSearch.addFilter('status', 'IN_LIST', '3');
									...
									refresh: function (oParam, oResponse)
												{
													...
													else if (iStep == 3)
													{
														...
														if ((cAmount - cAllocated) <= 0)
														{
															...
															{
																if ($('#ns1blankspaceReconcileItems td.ns1blankspaceReconcileItemsMatch:visible').length == 1)
																{
																	var aHTML = [];
									...
									edit:		function (oParam, oResponse)
												{
													...
													if ($('#' + sXHTMLElementContainerID).html() == '')
													{
														...
														if (ns1blankspace.option.bootstrap)
														{
															aHTML.push('<div class="btn-group" role="group" data-toggle="buttons" id="ns1blankspaceReconcileItemEditOption" style="margin-top:3px; text-align:right;">');

															if (iType == 1)  //DEBITS (OUT)
															{	
																aHTML.push('<label class="btn btn-default btn-sm" id="ns1blankspaceReconcileItemEdit-1">' +
																					'<input type="radio" name="radioEdit" data-1blankspace="ignore" />Payments & Journals' +
																					'</label>');

																aHTML.push('<label class="btn btn-default btn-sm" id="ns1blankspaceReconcileItemEdit-2">' +
																					'<input type="radio" name="radioEdit" data-1blankspace="ignore" />Outstanding Expenses' +
																					'</label>');
															}
															else if (iType == 2) //CREDITS (IN)
															{
																aHTML.push('<label class="btn btn-default btn-sm" id="ns1blankspaceReconcileItemEdit-1">' +
																					'<input type="radio" name="radioEdit" data-1blankspace="ignore" />Receipts & Journals' +
																					'</label>');

																aHTML.push('<label class="btn btn-default btn-sm" id="ns1blankspaceReconcileItemEdit-2">' +
																					'<input type="radio" name="radioEdit" data-1blankspace="ignore" />Unpaid Invoices' +
																					'</label>');
															}
														...
														{
															aHTML.push('<div id="ns1blankspaceReconcileItemEditOption" style="margin-bottom:6px;">');
														...
														if (!ns1blankspace.option.bootstrap)
														{
															$('#ns1blankspaceReconcileItemEditOption').buttonset().css('font-size', '0.825em');
															sSelect = ':radio'
														}
														else
														{
															$('#ns1blankspaceReconcileItemEdit-' + iSource).addClass('active');
														}

														$('#ns1blankspaceReconcileItemEdit-' + iSource).attr('checked', 'checked');														
														$('#ns1blankspaceReconcileItemEditOption ' + sSelect).click(function()
														{
															var aID = (this.id).split('-');
													...
													if (iEditAction == 1)  //SET UP SEARCH PAYMENT OR RECEIPT +++
													{
														...
														{	
															...
															aHTML.push('<tr><td style="padding:0px; padding-left:6px; padding-top:8px; text-align:center;">' +		
																				'<input type="checkbox" id="ns1blankspaceItemsEditAddAsUnallocated" data-1blankspace="ignore" style="margin:0px; padding:0px; border: 0px; margin-top:1px;">' +
																				'<span style="font-weight:100; font-size:0.825em; margin-left:2px;" class="ns1blankspaceSub">Set as Unallocated</span>' +
																				'</td></tr>');
														...
														.click(function()
														{
															var oData = 
															{
																date: ns1blankspace.util.getData(oParam, 'data-date', {param: 'sourceXHTMLElementID'}).value,
																amount: ns1blankspace.util.getData(oParam, 'data-unallocatedAmount', {param: 'sourceXHTMLElementID'}).value,
																description: ns1blankspace.util.getData(oParam, 'data-description', {param: 'sourceXHTMLElementID'}).value,
																itemDescription: ns1blankspace.util.getData(oParam, 'data-description', {param: 'sourceXHTMLElementID'}).value,
																account: ns1blankspace.util.getData(oParam, 'data-financialaccount', {param: 'sourceXHTMLElementID'}).value,
																contactBusiness: ns1blankspace.util.getData(oParam, 'data-contactbusiness', {param: 'sourceXHTMLElementID'}).value,
																contactPerson: ns1blankspace.util.getData(oParam, 'data-contactperson', {param: 'sourceXHTMLElementID'}).value,
																taxType: ns1blankspace.util.getData(oParam, 'data-taxtype', {param: 'sourceXHTMLElementID'}).value
															}

															$.extend(true, oParam, {editAction: 4});

															if (oData.date != '' && oData.amount != '' && oData.description != '' && oData.account != ''
																		&& (oData.contactBusiness != '' || oData.contactPerson != '') && oData.taxType != '' )
															{
																...
																if ($('#ns1blankspaceItemsEditAddAsUnallocated').prop('checked') == true)
																{
																	if (iObject == 2 || iObject == 3)
																	{
																		oData.account = ns1blankspace.financial.data.settings.financialaccountunallocated.outgoing
																	}
																	else
																	{
																		oData.account = ns1blankspace.financial.data.settings.financialaccountunallocated.incoming;
																	}
																}

																oParam = $.extend(true, oParam, oData);	
													...
													else if (iEditAction == 4) //MANUALLY ADD PAYMENT OR RECEIPT
													{
														if (oResponse == undefined)
														{
															...
															aHTML.push('<tr><td style="padding-left:2px; padding-top:10px;">' +
																			'<label for="ns1blankspaceReconcileItemsEditRemember" style="font-weight:normal;">Remember as mapping</label>' +
															...
															.click(function()
															{
																...
																oParam = ns1blankspace.util.setParam(oParam, 'itemDescription', $('#ns1blankspaceItemDescription').val());
									...
									locked: 	function(oParam, oResponse)
												{
													...
													{	
														...
														if (ns1blankspace.option.bootstrap)
														{
															aHTML.push('<div class="btn-group" role="group" data-toggle="buttons" id="ns1blankspaceReconcileItemLockedOption" style="margin-bottom:6px;">');

															if (iType == 1)  //DEBITS (OUT)
															{	
																aHTML.push('<label class="btn btn-default" id="ns1blankspaceReconcileItemLocked-1">' +
																					'<input type="radio" name="radioLockedSource" data-1blankspace="ignore" />Payments' +
																					'</label>');
															}
															else if (iType == 2) //CREDITS (IN)
															{
																aHTML.push('<label class="btn btn-default" id="ns1blankspaceReconcileItemLocked-1">' +
																					'<input type="radio" name="radioLockedSource" value="1" data-1blankspace="ignore" />Receipts' +
																					'</label>');
															}

															aHTML.push('<label class="btn btn-default" id="ns1blankspaceReconcileItemLocked-3">' +
																					'<input type="radio" name="radioLockedSource" value="1" data-1blankspace="ignore" />Journals' +
																					'</label>');
														...
														{
															aHTML.push('<div id="ns1blankspaceReconcileItemLockedOption" style="margin-bottom:3px;text-align:left;">');
														...
														if (!ns1blankspace.option.bootstrap)
														{
															$('#ns1blankspaceReconcileItemLockedOption').buttonset().css('font-size', '0.825em');
															sSelect = ':radio'
														}
														else
														{
															$('#ns1blankspaceReconcileItemLocked-' + iSource).addClass('active');
														}

														$('#ns1blankspaceReconcileItemLocked-' + iSource).attr('checked', 'checked');
														
														$('#ns1blankspaceReconcileItemLockedOption ' + sSelect).click(function()
														{
															var aID = (this.id).split('-');
									...
									send: 	function(oParam)
												{
													if (oParam == undefined)
													{
														oParam =
														{
															...
															searchDescription: $('#ns1blankspaceBankAccountTransferDescription').val(),
													...
													else
													{
														...
														if (oParam.searchDescription != '' && oParam.searchDescription != undefined)
														{
															oData.description = oData.description + ' (' + oParam.searchDescription + ')'
														}
...
ns1blankspace.financial.bankAccount.refresh = function (oParam, oResponse)
{
	if (oResponse == undefined)
	{
		var oSearch = new AdvancedSearch();
		...
		oSearch.addFilter('status', 'EQUAL_TO', 1)
		oSearch.rows = 100;
		oSearch.getResults(function(data) {ns1blankspace.financial.bankAccount.refresh(oParam, data)});
		
	}
	else
	{
		ns1blankspace.financial.data.bankaccounts = oResponse.data.rows;
		ns1blankspace.onComplete(oParam);
	}
}
