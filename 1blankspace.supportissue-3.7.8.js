	...
	home: function (oParam, oResponse)
	{
		...
		if (oResponse == undefined)
		{
			...
			else if (oParam.issueHomeStep == 3)
			{
				...
				/// v2.0.1 Only applies to spaceAdmin users
				if (ns1blankspace.user.spaceAdmin)
				{
					...
					aHTML.push('<tr><td style="padding-top:20px;" id="ns1blankspaceSupportIssueSpaceAdmin"></td></tr>');	
					...
					aHTML.push('<tr><td id="ns1blankspaceSupportDebugLogShowContainer">' +
									'<span id="ns1blankspaceSupportDebugLogShow" style="width:20px; height:20px;"></span>' +
									'<span id="ns1blankspaceSupportDebugLogClear" style="margin-left:4px; width:20px; height:20px;"></span>' +
									'</td></tr>');
				}
				...
				if (ns1blankspace.supportIssue.data.acceptsSupportIssues == true &&
						(ns1blankspace.util.hasAccess({method: 'SETUP_SUPPORT_ISSUE_STATUS_SEARCH'}) ||
							ns1blankspace.util.hasAccess({method: 'SETUP_SUPPORT_ISSUE_VERSION_SEARCH'})));
				{
					aHTML.push('<tr><td>&nbsp;</td></tr>');	
					...
					if (ns1blankspace.util.hasAccess({method: 'SETUP_SUPPORT_ISSUE_STATUS_SEARCH'}))
					{
						aHTML.push('<tr class="ns1blankspaceControl">' +
					...
					if (ns1blankspace.util.hasAccess({method: 'SETUP_SUPPORT_ISSUE_VERSION_SEARCH'}))
					{
						aHTML.push('<tr class="ns1blankspaceControl">' +
			...
			else if (oParam.issueHomeStep == 4)
			{
				...
				var sFields = 'completeddate,contactbusiness,contactbusinesstext,contactperson,contactpersontext,' +
								'description,email,lodgeddate,name,phone,processingtype,processingtypetext,reference,' +
								'severity,severitytext,solution,status,statustext,title,' +
				...
				{
					sFields = sFields + ',technicalnotes'
				}
		...
		else
		{
			...
			else
			{
				...
				$.each(oResponse.data.rows, function()
				{
                    ...
                    aHTML.push('<td id="ns1blankspaceMostLikely_Contact-' + this.id + '" class="ns1blankspaceRow" style="width:100px;">' +
	...
	summary:	function ()
	{
		...
		else
		{
			...
			if (ns1blankspace.objectContextData.subject != '')
			{
                ...
                aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Lodged By</td></tr>' +
								'<tr><td id="ns1blankspaceSummaryCreatedUser" class="ns1blankspaceSummary">' +
								ns1blankspace.objectContextData.createdusertext +
								'</td></tr>');
                ...
                aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Last Updated By</td></tr>' +
								'<tr><td id="ns1blankspaceSummaryModifiedUser" class="ns1blankspaceSummary">' +
								ns1blankspace.objectContextData.modifiedusertext +
								'</td></tr>');
	...
	issuesByStatus: 
	{
		...
		show: function(oParam, oResponse)
		{
			...
			if ($('#' + sXHTMLElementID).attr('data-loading') === '1') 
			{
				...
				aHTML.push('<tr><td><div id="ns1blankspace' + sXHTMLContext + 'Search" class="ns1blankspaceAction">Search</div></td></tr>');
