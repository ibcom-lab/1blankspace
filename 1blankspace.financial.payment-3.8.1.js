					...
					if (oResponse == undefined)
					{
						...
						aHTML.push('<tr class="ns1blankspaceControl">' +
									'<td style="padding-top:18px;" id="ns1blankspaceControlUnallocated" class="ns1blankspaceControl">' +
										'<div>Unallocated</div><div class="ns1blankspaceSub" style="font-size:0.625em;">Payment items that have been marked as unallocated and not applied to an expense.</div></td>' +
									'</tr>');

						aHTML.push('<tr class="ns1blankspaceControl">' +
									'<td style="padding-top:18px;" id="ns1blankspaceControlUnreconciled" class="ns1blankspaceControl">' +
										'<div>Unreconciled</div><div class="ns1blankspaceSub" style="font-size:0.625em;">Payments that have not been reconciled against a bank account.</div></td>' +
									'</tr>');
					...
					process:	function (oParam, oResponse)
								{
									...
									{		
										...
										$.each(oResponse.data.rows, function()
										{	
											aHTML.push(ns1blankspace.financial.payment.search.row(this, oParam));
										});
					...
					amount:	function (oParam)
					{
						...
						if (cAmount != undefined)
						{
							...
							else
							{
								...
								sData += '&description=' + ns1blankspace.util.fs($('#ns1blankspaceDetailsDescription').val());
								sData += '&taxtype=' + $('[name="radioTaxCode"]:checked').val();
											...
											$.ajax(
											{
												...
												success: function (data)
												{
													if (data.status == 'OK')
													{
														...
														ns1blankspace.financial.payment.summary.show();		
													}
													else
													{
						...
						else
						{
							...
							else
							{
								...
								$.each(oResponse.data.rows, function()
								{
									...
									aHTML.push('<td id="ns1blankspaceFinancialPaymentsUnallocated_notes-' + this['id'] + '" class="ns1blankspaceRow">' +
													this['description'] + '</td>');

									aHTML.push('<td id="ns1blankspaceFinancialPaymentsUnallocated_date-' + this['id'] + '" class="ns1blankspaceRow" style="width:100px;">' +
					...
					row: 			function (oRow, oParam)	
									{
										...
										aHTML.push('<td id="ns1blankspaceFinancialPaymentsUnreconciled_notes-' + oRow.id + '" class="ns1blankspaceRow">' +
														oRow.notes + '</td>');

										aHTML.push('<td id="ns1blankspaceFinancialPaymentsUnreconciled_date-' + oRow.id + '" class="ns1blankspaceRow" style="width:100px;">' +
