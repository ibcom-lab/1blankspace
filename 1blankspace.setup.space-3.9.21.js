	...
	home:		function (oParam, oResponse)
				{
					...
					else
					{
						...
						aHTML.push('<tr class="ns1blankspaceControl">' +
										'<td id="ns1blankspaceControlStorage" class="ns1blankspaceControl">Other Services Accounts' +
										'<br /><span class="ns1blankspaceSub" style="font-size:0.75em;">eg AWS</span></td>' +
										'</tr>');
					...
					show: 	function (oParam)
								{
									...
									else
									{
                                        ...
                                        if (oTemplate.structures != 0 && (iType == 5 || iType == -1))
										{
											aHTML.push('<input type="radio" id="ns1blankspaceInitaliseOption-financials" name="radioObject" />' +
														'<label for="ns1blankspaceInitaliseOption-financials" style="width: 110px; margin-bottom:3px;">' +
														'Financial Accounts</label>');
										}	
									...
									show:		function (oParam, oResponse)
												{
													...
													{	
														...
														$(aRolesRequired).each(function(i, v) 
														{
															...
															{
																aHTML.push('<td style="width:350px;" class="ns1blankspaceRow">' +
																			'<span id="ns1blankspaceSetupSpaceInitialiseRoles_options_merge-' + i + '" class="ns1blankspaceRowMerge" data-title="' + this.title + '"' +
																				' data-id="' + aRole[0].id + '"' +
																			'></span></td>');
														...
														$('#ns1blankspaceSetupSpaceInitialiseRoles .ns1blankspaceRowMerge').button(
														{
															label: 'Merge Role'
														})
														.click(function()
														{
															ns1blankspace.setup.space.initialise.roles.add({xhtmlElementID: this.id, merge: true})
														})
														.css('font-size', '0.725em')
														.css('height', '28px');
									...
									add: 		function(oParam, oResponse)
												{
													...
													if (oParam.roleMethodsExisting != undefined) {aRoleMethodsExisting = oParam.roleMethodsExisting}
													...
													if (oParam.merge != undefined) {bMerge = oParam.merge}
													...
													if (iStep == 1)
													{
														...
														if (bMerge)
														{
															var iID = $('#' + sXHTMLElementID).attr('data-id');
															oParam.role = iID;

															var oSearch = new AdvancedSearch();
															oSearch.method = 'SETUP_ROLE_METHOD_ACCESS_SEARCH';
															oSearch.addField('accessmethod,accessmethodtext');
															oSearch.addFilter('role', 'EQUAL_TO', iID);
															oSearch.rows = 1000;
															oSearch.sort('accessmethodtext', 'asc');
															oSearch.getResults(function(oResponse)
															{
																oParam.roleMethodsExisting = oResponse.data.rows;
													...
													if (iStep == 4)
													{
														if (iRoleMethod < aRoleMethods.length)
														{
															...
															var aExistingMethod = $.grep(aRoleMethodsExisting, function (a) { return a.accessmethodtext == aRoleMethods[iRoleMethod].title});

															if (aExistingMethod.length != 0)
															{
																ns1blankspace.status.working('Merging method ' + (iRoleMethod + 1) + ' of ' + aRoleMethods.length);
																oData.id = aExistingMethod[0].id;
															}
															else
															{
                        ...
                        financials: 	
                                {
                                    ...
                                    show:	function (oParam, oResponse)
                                                    {
                                                        ...
                                                        ns1blankspace.setup.space.initialise.financials.data.accounts = [];
    
                                                        var aHTML = [];
    
                                                        aHTML.push('<table id="ns1blankspaceSetupSpaceInitialiseFinancialsContainer" class="ns1blankspaceColumn2">');
                                                
                                                        aHTML.push('<tr><td id="ns1blankspaceSetupSpaceInitialiseFinancials">');
                                                        ...
                                                        $('#ns1blankspaceUpload').button(
                                                        {
                                                            label: "Import Financial Accounts"
                                                        })
                                                        .click(function()
                                                        {
                                                            if ($('#oFile0').val() == '')
                                                            ...
                                                            else
                                                            {
                                                                $.ajax(
                                                                {
                                                                    ...
                                                                    success: function(data)
                                                                    {
                                                                        ...
                                                                        {	
                                                                            $('#objectcontext').val(data.id);	
                                                                            ns1blankspace.attachments.upload.submit({submit: true, functionPostUpdate: ns1blankspace.setup.space.initialise.financials.import});
                                                                        }
                                                                    }
                                                                });
                                                            }
                                    ...
                                    import: 	function(oParam)
                                                    {
                                                        ...
                                                        $.ajax(
                                                        {
                                                            ...
                                                            success: function(data)
                                                            {
                                                                var oFinancialsTemplate = JSON.parse(data.filedata);
                                                                ns1blankspace.setup.space.initialise.data.template.financials = oFinancialsTemplate.template.financials
                                                                ns1blankspace.setup.space.initialise.financials.add()
                                                            }
                                                        });
                                                    },
                                    ...
                                    add: 		    function(oParam, oResponse)
                                                    {
                                                        var iStep = 1;
                                                        var iAccountToBeProcessedIndex = 0

                                                        if (oParam == undefined) {oParam = {}}
                                                        if (oParam.step != undefined) {iStep = oParam.step}
                                                        if (oParam.xhtmlElementID != undefined) {sXHTMLElementID = oParam.xhtmlElementID}
                                                        if (oParam.accountToBeProcessedIndex != undefined) {iAccountToBeProcessedIndex = oParam.accountToBeProcessedIndex }
                                                       
                                                        var aImportAccounts = ns1blankspace.setup.space.initialise.data.template.financials.accounts;
                                                        var iImportRootAccountID = ns1blankspace.setup.space.initialise.data.template.financials.root.account;
                                                        var aThisSpaceAccounts = ns1blankspace.setup.space.initialise.financials.data.thisSpaceAccounts;

                                                        if (iStep == 1)
                                                        {
                                                            ns1blankspace.status.working();
    
                                                            $('#ns1blankspaceSetupSpaceInitialiseStructures').html('<div class="ns1blankspaceSub">Importing Accounts...</div>');
                                                            ...
                                                            if (oResponse == undefined)
                                                            {
                                                                ...
                                                                oSearch.rows = 99999;
                                                                oSearch.getResults(function(data) {ns1blankspace.setup.space.initialise.financials.add(oParam, data)})	
                                                            }
                                                            else
                                                            {
                                                                if (oResponse.data.rows.length <= 6)
                                                                {
                                                                    ns1blankspace.status.message('You need to intialise the financial accounts.')
                                                                }
                                                                else
                                                                {
                                                                    ns1blankspace.setup.space.initialise.financials.data.thisSpaceAccounts = oResponse.data.rows;
                                                                    oParam.step = 2
                                                        ...
                                                        { 
                                                            ...
                                                            ns1blankspace.status.working('Importing accounts... ');

                                                            console.log('IMPORT ACCOUNTS:')
                                                            console.log(console.log(aImportAccounts))

                                                            //Get root from import
                                                            //Get root from existing

                                                            var aThisSpaceAccountsRoot = $.grep(aThisSpaceAccounts, function(account) {return account.parentaccount == ''});
                                                            var iThisSpaceRootAccountID = aThisSpaceAccounts[0].id;
                                                            var sThisSpaceRootAccountText = aThisSpaceAccounts[0].title;
                                                            
                                                            $.each(aImportAccounts, function (a, importAccount)
                                                            {
                                                                if (importAccount.title == sThisSpaceRootAccountText)
                                                                {
                                                                    importAccount._thisSpaceAccountRoot = aThisSpaceAccountsRoot[0];
                                                                }
                                                            });

                                                            $.each(aThisSpaceAccounts, function (a, thisSpaceAccount)
                                                            {
                                                                thisSpaceAccount.title = thisSpaceAccount.title.formatXHTML();
                                                                thisSpaceAccount.parentaccounttext = thisSpaceAccount.parentaccounttext.formatXHTML();

                                                                thisSpaceAccount._importAccount = $.grep(aImportAccounts, function (importAccount)
                                                                {
                                                                    return (thisSpaceAccount.title.toLowerCase() == importAccount.title.toLowerCase() 
                                                                        && thisSpaceAccount.type == importAccount.type )
                                                                });
                                                            });

                                                            console.log('THIS SPACE ACCOUNTS:')
                                                            console.log(aThisSpaceAccounts)

                                                            // Get type roots; Expense, Reveue, Asset, Liability, Equity

                                                            var aImportAccountsTypeRoots = $.grep(aImportAccounts, function(account) {return account.parentaccount == iImportRootAccountID});
                                                            var aThisSpaceAccountsTypeRoots = $.grep(aThisSpaceAccounts, function(account) {return account.parentaccount == iThisSpaceRootAccountID});
                                                            
                                                            var aImportAccountsToBeProcessed = [];
                                                            var aImportAccountsProcessed = [];

                                                            $.each(aImportAccounts, function (a, importAccount)
                                                            {
                                                                importAccount.title = importAccount.title.formatXHTML();
                                                                importAccount.parentaccounttext = importAccount.parentaccounttext.formatXHTML();

                                                                importAccount._thisSpaceAccount = $.grep(aThisSpaceAccounts, function (thisSpaceAccount)
                                                                {
                                                                    return (importAccount.title.toLowerCase() == thisSpaceAccount.title.toLowerCase()
                                                                        && importAccount.type == thisSpaceAccount.type)
                                                                });

                                                                if (importAccount._thisSpaceAccount.length == 0)
                                                                {
                                                                    aImportAccountsToBeProcessed.push(importAccount)
                                                                }
                                                                else
                                                                {
                                                                    aImportAccountsProcessed.push(importAccount)
                                                                }
                                                            });

                                                            console.log('IMPORT ACCOUNTS TO BE PROCESSED:')
                                                            console.log(aImportAccountsToBeProcessed)
                                                            ns1blankspace.setup.space.initialise.financials.data.importAccountsToBeProcessed = aImportAccountsToBeProcessed;

                                                            console.log('IMPORT ACCOUNTS PROCESSED:')
                                                            console.log(aImportAccountsProcessed)
                                                            ns1blankspace.setup.space.initialise.financials.data.importAccountsProcessed = aImportAccountsProcessed;

                                                            ns1blankspace.setup.space.initialise.financials.data.importAccountsToBeSaved = [];
                                                        ...
                                                        if (iStep == 3)
                                                        { 
															// loop through each in processed to see if parent is in the processed - and repeat

                                                            if (iAccountToBeProcessedIndex < ns1blankspace.setup.space.initialise.financials.data.importAccountsToBeProcessed.length)
                                                            {
                                                                var oAccountToBeProcessed = ns1blankspace.setup.space.initialise.financials.data.importAccountsToBeProcessed[iAccountToBeProcessedIndex];

                                                                ns1blankspace.status.working('Processing ' + oAccountToBeProcessed.title);
                                                                
                                                                oAccountToBeProcessed._parentAccountProcessed = $.grep(ns1blankspace.setup.space.initialise.financials.data.importAccountsProcessed, 
                                                                        function (oProcessedAccount)
                                                                        {
                                                                            return  (   (
                                                                                            oAccountToBeProcessed.parentaccounttext.toLowerCase() == oProcessedAccount.title.toLowerCase()
                                                                                            || oAccountToBeProcessed.code.toLowerCase() == oProcessedAccount.code.toLowerCase()
                                                                                        ) 
                                                                                        && (oAccountToBeProcessed.type == oProcessedAccount.type)
                                                                                        && oProcessedAccount._thisSpaceAccount.length != 0
                                                                                    )
                                                                        });

                                                                if (oAccountToBeProcessed._parentAccountProcessed.length != 0)
                                                                {
                                                                    ns1blankspace.setup.space.initialise.financials.data.importAccountsToBeSaved.push(oAccountToBeProcessed);

                                                                    var oData = 
                                                                    {
                                                                        area: oAccountToBeProcessed.area,
                                                                        code: oAccountToBeProcessed.code,
                                                                        description: oAccountToBeProcessed.description,
                                                                        postable: oAccountToBeProcessed.postable,
                                                                        title: oAccountToBeProcessed.title,
                                                                        type: oAccountToBeProcessed.type,
                                                                        parentaccount: oAccountToBeProcessed._parentAccountProcessed[0]._thisSpaceAccount[0].id
                                                                    }

                                                                    if (oData.type == 1)
                                                                    {
                                                                        oData.class = oAccountToBeProcessed.class;
                                                                        oData.expensecostofsale = oAccountToBeProcessed.expensecostofsale;
                                                                        oData.expensepayroll = oAccountToBeProcessed.expensepayroll;
                                                                    }

                                                                    if ((oData.type == 1 || oData.type == 2) && oData.postable == 'Y')
                                                                    {
                                                                        if (oAccountToBeProcessed.taxtype != '')
                                                                        {
                                                                            oData.taxtype = oAccountToBeProcessed.taxtype;
                                                                        }
                                                                    }
                                                                ...
                                                                {
                                                                    oParam.accountToBeProcessedIndex = iAccountToBeProcessedIndex + 1;
                                                                    ns1blankspace.setup.space.initialise.financials.add(oParam)
                                                                }
                                                            }
                                                            else
                                                            {
                                                                ns1blankspace.setup.space.initialise.financials.settings.init(oParam);
                                                            }	
                                                        }	
                                                    },
                                        ...
                                        settings:   {
                                                        init: function(oParam, oResponse)
                                                        {
                                                            if (oResponse == undefined)
                                                            {
                                                                ...
                                                                oSearch.addField('financialaccountcashtext,' +
                                                                                    'financialaccountcreditorstext,' +
                                                                                    'financialaccountcurrentearningstext,' +
                                                                                    'financialaccountdebtorstext,' +
                                                                                    'financialaccountfinancialservicescommissiontext,' +
                                                                                    'financialaccountproductincometext,' +
                                                                                    'financialaccountproductinventorytext,' +
                                                                                    'financialaccountproductpurchasestext,' +
                                                                                    'financialaccountretainedearningstext,' +
                                                                                    'financialaccountsalescommissiontext,' +
                                                                                    'financialaccountwriteofftext,' +
                                                                                    'payrollfinancialaccountallowancetext,' +
                                                                                    'payrollfinancialaccountsalarytext,' +
                                                                                    'payrollfinancialaccountsuperannuationtext,' +
                                                                                    'payrollfinancialaccounttaxtext');
                                                                oSearch.rows = 1;
                                                                oSearch.getResults(function(data) {ns1blankspace.setup.space.initialise.financials.settings.init(oParam, data)})	
                                                            }
                                                            else
                                                            {
                                                                ns1blankspace.setup.space.initialise.financials.data.settings = {};

                                                                if (oResponse.data.rows.length != 0)
                                                                {
                                                                    ns1blankspace.setup.space.initialise.financials.data.settings = oResponse.data.rows[0];
                                                                }

                                                                ns1blankspace.setup.space.initialise.financials.settings.save(oParam);
                                                            }
                                                        },
                                        
                                                        save: function()
                                                        {
                                                            //SETUP_FINANCIAL_SETTINGS_MANAGE
                                                            var thisSpaceSettings = {id: ns1blankspace.setup.space.initialise.financials.data.settings.id};
                                                            var aThisSpaceAccounts = ns1blankspace.setup.space.initialise.financials.data.thisSpaceAccounts;

                                                            _.each(ns1blankspace.setup.space.initialise.data.template.financials.settings, function (value, setting)
                                                            {
                                                                if (value != '')
                                                                {
                                                                    var settingThisSpaceAccount = $.grep(aThisSpaceAccounts, function (thisSpaceAccount)
                                                                    {
                                                                        return (value.toLowerCase() == thisSpaceAccount.title.toLowerCase())
                                                                    });

                                                                    if (settingThisSpaceAccount.length != 0)
                                                                    {
                                                                        if (ns1blankspace.setup.space.initialise.financials.data.settings[setting] == '')
                                                                        {
                                                                            thisSpaceSettings[setting.replace('text', '')] = settingThisSpaceAccount[0].id
                                                                        }
                                                                    }
                                                                }
                                                            });

                                                            console.log('SETTINGS TO BE SAVED:')
                                                            console.log(thisSpaceSettings)

                                                            $.ajax(
                                                            {
                                                                type: 'POST',
                                                                url: '/rpc/setup/?method=SETUP_FINANCIAL_SETTINGS_MANAGE',
                                                                data: thisSpaceSettings,
                                                                dataType: 'json',
                                                                success: function(data)
                                                                {
                                                                    ns1blankspace.status.message('Financial accounts imported');
                                                                    ns1blankspace.setup.financial.init({initialised: false})
                                                                }
                                                            });
                                                        }
                                                    }
								...
								{	
									...
									{	
                                        ...
                                        aHTML.push('<tr><td style="padding-top:20px;" class="ns1blankspaceSubNote">' +
													'You can access AWS lambda, dynamoDB & S3 services by setting up an access account.' +
													'</td></tr>');

                                                    aHTML.push('<tr><td style="padding-top:10px;" class="ns1blankspaceSubNote">' +
													'You can then use the <a href="https://docs.mydigitalstructure.cloud/endpoint_core" target="_blank">core endpoint</a> to use the services with the <em>Account ID</em>.' +
													'</td></tr>');
													
										aHTML.push('<tr><td style="padding-top:20px;">' +
													'<a href="https://aws.amazon.com/s3/" target="_blank">Storing Files on AWS S3</a>' +
													'</td></tr>');	
										...
										else
										{
                                            ...
                                            aHTML.push('<td class="ns1blankspaceHeaderCaption" style="text-align:center;">Account ID</td>');
											aHTML.push('<td class="ns1blankspaceHeaderCaption">&nbsp;</td>');
											aHTML.push('</tr>');
											...
											$.each(oResponse.data.rows, function()
											{
                                                ...
                                                aHTML.push('<td id="ns1blankspaceSpaceStorage_id-' + this.id + '" class="ns1blankspaceRow ns1blankspaceSub" style="text-align:center;">' +
																		this.id + '</td>');
...
ns1blankspace.setup.space.export.financial =
{
    accounts:
    {
        data: {},
        ...
        init: function (oParam, oResponse)
        {
            ...
            if (oResponse == undefined)
            {
                ...
                oSearch.sort('title', 'asc')
                oSearch.rows = 9999;
                oSearch.getResults(function(data) {ns1blankspace.setup.space.export.financial.accounts.init(oParam, data)})	
            }
            else
            {
                if (oResponse.data.rows.length != 0)
                {
                    ns1blankspace.setup.space.export.financial.accounts.data.accounts = oResponse.data.rows;

					_.each(ns1blankspace.setup.space.export.financial.accounts.data.accounts, function (account)
					{
						account.title = account.title.replace('\r\n', ' ');
                        account.parentaccounttext = account.parentaccounttext.replace('\r\n', ' ');
                        account.typetext = account.typetext.replace('\r\n', ' ');
					});

                    ns1blankspace.setup.space.export.financial.accounts.settings(oParam);
                }
            }
        },

		settings: function (oParam, oResponse)
        {
            if (oResponse == undefined)
            {
                var oSearch = new AdvancedSearch();
            ...
            else
            {
                if (oResponse.data.rows.length != 0)
                {
                    ns1blankspace.setup.space.export.financial.accounts.data.settings = oResponse.data.rows[0];
                    ns1blankspace.setup.space.export.financial.accounts.process(oParam);
                }
            }
        },
        ...
        process: function (oParam, oResponse)
        {
            var exportAccountsRootAccount = $.grep(ns1blankspace.setup.space.export.financial.accounts.data.accounts,
                function(account) {return account.parentaccount == ''});

            if (exportAccountsRootAccount.length != 0)
            {
                exportAccountsRootAccount = exportAccountsRootAccount[0].id;
            }

            var exportAccounts = ns1blankspace.setup.space.export.financial.accounts.data.accounts;

            ns1blankspace.setup.space.export.financial.accounts.data.file = [];
            var aFile = ns1blankspace.setup.space.export.financial.accounts.data.file;

            aFile.push('{');
            aFile.push('\t"template":');
            aFile.push('\t{');
            aFile.push('\t\t"financials":');
            aFile.push('\t\t{');
            aFile.push('\t\t\t"root":');
            aFile.push('\t\t\t{');
            aFile.push('\t\t\t\t"account": "' + exportAccountsRootAccount + '"');
            aFile.push('\t\t\t},');
            aFile.push('\t\t\t"accounts":');
            aFile.push('\t\t\t[');
            ...
            exportAccounts = $.grep(ns1blankspace.setup.space.export.financial.accounts.data.accounts,
                    function(account) {return account.parentaccount != ''});

            //exportAccounts = $.grep(exportAccounts,
            //        function(account) {return account.parentaccount != exportAccountsRootAccount});
    
            $.each(exportAccounts, function (a, account)
            {
                account.code = account.code.formatXHTML();
                account.title = account.title.formatXHTML();
                account.description = account.description.formatXHTML();
                account.parentaccounttext = account.parentaccounttext.formatXHTML();
                account.taxtypeincomingtext = account.taxtypeincomingtext.formatXHTML();
                account.taxtypeoutgoingtext = account.taxtypeoutgoingtext.formatXHTML();
                account.areatext = account.areatext.formatXHTML();
                account.typetext = account.typetext.formatXHTML();
                account.classtext = account.classtext.formatXHTML();

                aFileItems.push('\t\t\t\t{"title": "' + account.title + '", ' +
                                            ' "description": "' + account.description + '",' +
                                            ' "code": "' + account.code + '",' +
                                            ' "type": "' + account.type + '",' +
                                            ' "typetext": "' + account.typetext + '",' +
                                            ' "class": "' + account.class + '",' +
                                            ' "classtext": "' + account.classtext + '",' +
                                            ' "expensecostofsale": "' + account.expensecostofsale + '",' +
                                            ' "expensepayroll": "' + account.expensepayroll + '",' +
                                            ' "parentaccount": "' + account.parentaccount + '",' +
                                            ' "parentaccounttext": "' + account.parentaccounttext + '",' +
                                            ' "postable": "' + account.postable + '",' +
                                            ' "taxtype": "' + account.taxtype + '",' +
                                            ' "taxtypeincomingtext": "' + account.taxtypeincomingtext + '",' +
                                            ' "taxtypeoutgoingtext": "' + account.taxtypeoutgoingtext + '",' +
                                            ' "area": "' + account.area + '",' +
                                            ' "areatext": "' + account.areatext + '"' +
                                            '}');
            });
            ...
            {
                data: aFile.join('\n'),
                filename: 'setup-financial-accounts.json'
            });
        }
    }
}
