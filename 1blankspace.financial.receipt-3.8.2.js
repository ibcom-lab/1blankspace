...
ns1blankspace.financial.receipt = 
{
					...
					if (oResponse == undefined)
					{
						...
						aHTML.push('<tr class="ns1blankspaceControl">' +
									'<td style="padding-top:18px;" id="ns1blankspaceControlUnallocated" class="ns1blankspaceControl">' +
										'<div>Unallocated</div><div class="ns1blankspaceSub" style="font-size:0.625em;">Receipt items that have been marked as unallocated and not applied to an invoice.</div></td>' +
									'</tr>');

						aHTML.push('<tr class="ns1blankspaceControl">' +
									'<td style="padding-top:18px;" id="ns1blankspaceControlUnreconciled" class="ns1blankspaceControl">' +
										'<div>Unreconciled</div><div class="ns1blankspaceSub" style="font-size:0.625em;">Receipts that have not been reconciled against a bank account.</div></td>' +
									'</tr>');
					...
					else
					{
						...
						else
						{
							...
							{					
								...
								aHTML.push('<td id="ns1blankspaceMostLikely_Date-' + this.id + '" class="ns1blankspaceMostLikelySub" style="width:100px;">' +
					...
					process:	function (oParam, oResponse)
								{
									...
									{		
										...
										$.each(oResponse.data.rows, function()
										{	
											aHTML.push(ns1blankspace.financial.receipt.search.row(this, oParam));
										});
	...
	summary: 	function (oParam)
				{
					...
					else
					{
						...
						{	
							...
							oParam = ns1blankspace.util.setParam(oParam, 'onComplete', ns1blankspace.financial.receipt.summary);
						...
						else
						{
							...
							aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Confirmation</td></tr>');
							...
							{	
								...
								.click(function(event)
								{
									ns1blankspace.financial.receipt.summary({useTemplate: true});
								});
							}
							...
							.click(function(event)
							{
								...
								ns1blankspace.financial.receipt.email.init();
							});
						}	
					}	
				},
	...
	email: 	{
					init: 	function (oParam)
								{								
									oParam = ns1blankspace.util.setParam(oParam, 'object', ns1blankspace.object);
									oParam = ns1blankspace.util.setParam(oParam, 'onComplete', ns1blankspace.financial.receipt.email.render);
					...
					render: 	function (oParam, oResponse)
								{
									...
									if (iMode == 1)
									{
										ns1blankspace.financial.receipt.email.show(oParam);
									}
									else
									{
										ns1blankspace.financial.receipt.email.send(oParam);
									}	
								},		
					...
					show: 	function (oParam)
								{
									...
									$('#ns1blankspaceEmailSubject').val(ns1blankspace.user.contactBusinessText + ' has receipted your payment of ' + ns1blankspace.objectContextData['amount']);
											...
											$.ajax(
											{
												...
												success: function (data)
												{
													if (data.status == 'OK')
													{
														...
														ns1blankspace.financial.receipt.summary.show();
													}
													else
													{
