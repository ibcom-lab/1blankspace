...
ns1blankspace.financial.invoice = 
{
	...
	util: 	{
					render: function (oParam)
					{
						...
						var oData = ns1blankspace.util.getParam(oParam, 'data', {"default": ns1blankspace.objectContextData}).value;
						var sReturn = sTemplate;

						if (sTemplate != undefined && oData != undefined)
						{
							sReturn = sReturn.replace('[[reference]]', oData.reference);
					...
					send: 	function (sXHTMLElementID, oParam)
								{
									...
									if (sSearchContext != undefined  && iSource != ns1blankspace.data.searchSource.browse)
									{
										...
										oSearch.addField('contactbusinesssenttotext,contactbusinesssentto,contactpersonsenttotext,contactpersonsentto,invoice.contactbusinesssentto.legalname,' +
														...
														'invoice.contactbusinesssentto.reference,invoice.contactpersonsentto.reference,status,statustext');
									...
									else
									{
										...
										if (sSearchText.length >= iMinimumLength || iSource == ns1blankspace.data.searchSource.browse)
										{
											...
											oSearch.addField('contactbusinesssenttotext,contactbusinesssentto,contactpersonsenttotext,contactpersonsentto,' +
																'reference,purchaseorder,sentdate,description,amount,status');
											...
											else
											{
												...
												if (ns1blankspace.objectContextData.contactpersonsenttotext != '')
												{
													aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Person</td></tr>' +
																	'<tr><td id="ns1blankspaceSummaryPerson" data-id="' + ns1blankspace.objectContextData.contactpersonsentto + '" data-object="contactPerson" class="ns1blankspaceSummary ns1blankspaceViewLink">' +
											...
											else
											{
												...
												$.each(ns1blankspace.format.templates.data[ns1blankspace.object], function (t, template)
												{
													$vq.add('<div class="ns1blankspaceRow ns1blankspaceRowSelect" style="border-width: 0px;"' +
																' id="ns1blankspaceViewTemplate-' + template.id + '">' + 
																		ns1blankspace.financial.invoice.util.render({template: template.title}) + 
																		'</div>', {queue: 'templates'});
												});	
									...
									else
									{
										...
										else
										{
											...
											else
											{
												...
												{
													$vq.add('<div class="ns1blankspaceRow ns1blankspaceRowSelect" style="border-width: 0px;"' +
																' id="ns1blankspaceEmailTemplate-' + template.id + '">' +
																ns1blankspace.financial.invoice.util.render({template:template.title}) +
																'</div>', {queue: 'templates'});
												});	
					...
					render: 	function (oParam, oResponse)
								{
									...
									{
										var oTemplate = ns1blankspace.format.templates.get(oParam);
										ns1blankspace.financial.invoice.data.template = oTemplate; 
					...
					show: 	function (oParam)
								{
									...
									var sSubject = ns1blankspace.user.contactBusinessText + ' Invoice ' + ns1blankspace.objectContextData['reference'];

									if (ns1blankspace.financial.invoice.data.template != undefined)
									{
										sSubject = ns1blankspace.financial.invoice.util.render({template: ns1blankspace.financial.invoice.data.template.title});
									}
	
									$('#ns1blankspaceEmailSubject').val(sSubject);

									if (ns1blankspace.financial.data.settings.messagingaccount != '')
									{
										$('#ns1blankspaceEmailFrom').val(ns1blankspace.financial.data.settings.messagingaccounttext);
									}
									else
									{
										$('#ns1blankspaceEmailFrom').val(ns1blankspace.user.email);
					...
					if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
					{
						...
						$('#ns1blankspaceDetailUndoSend').button().click(function()
						{
							...
							$.ajax(
							{
								...
								{
									ns1blankspace.financial.invoice.save.refresh();
									ns1blankspace.financial.invoice.search.send('-' + ns1blankspace.objectContext)
								}
							});
						});
					...
					send:		function (oParam, oResponse)
								{
									...
									{			
										...
										{	
											...
											ns1blankspace.financial.invoice.save.refresh(oParam);
										}
										else
										{
					...
					show: 	function (oParam, oResponse)
								{
									...
									{	
										...
										aHTML.push('<tr class="ns1blankspaceContainer">' +
														...
														'<td id="ns1blankspaceReceiptColumn2" style="width: 400px;">' +
										...
										var oSearch = new AdvancedSearch();
										oSearch.method = 'FINANCIAL_RECEIPT_INVOICE_SEARCH';
										oSearch.addField('appliesdate,amount,receiptlineitem,receiptinvoice.receipt.reference,receiptinvoice.receipt.amount,receiptinvoice.receipt.id');
									...
									else
									{
										...
										else
										{
											...
											$.each(oReceipts, function(r, receipt)
											{
												...
												if (oOptions.remove)
												{	
													aHTML.push('<span id="ns1blankspaceReceipt_options_remove-' + this.id + '-' + this.receiptlineitem + '" class="ns1blankspaceReceiptRemove"></span>');
												}
											...
											.click(function()
											{
												ns1blankspace.remove(
												{
													...
													caption: 'Remove',
													onComplete: ns1blankspace.financial.invoice.receipt.remove
												});
											})
											.css('width', '15px')
											.css('height', '17px');
					...
					allocate:
					 			{
					 				...
					 				init: function (oParam, oResponse)
					 				{
					 					if (oResponse == undefined)
					 					{
											...
											oSearch.addField('amount,object,financialaccount,description,' +
																	'lineitem.receipt.reference,lineitem.receipt.id,lineitem.receipt.contactbusinessreceivedfromtext,' +
																	'lineitem.receipt.contactpersonreceivedfromtext,' +
																	'lineitem.receipt.reference,lineitem.receipt.receiveddate,lineitem.receipt.amount');
											...
											{
												oSearch.addFilter('lineitem.receipt.contactbusinessreceivedfrom', 'EQUAL_TO', ns1blankspace.objectContextData.contactbusinesssentto);
											}
												
											if (ns1blankspace.objectContextData.contactpersonsentto != '')
											{
												oSearch.addFilter('lineitem.receipt.contactpersonreceivedfrom', 'EQUAL_TO', ns1blankspace.objectContextData.contactpersonsentto);
											}

											oSearch.getResults(function(data)
											{
												ns1blankspace.financial.invoice.receipt.allocate.init(oParam, data)
											});
										}
										else
										{
											...
											else
											{
												...
												$.each(oReceipts, function(i, aReceipts)
												{
													aHTML.push('<tr class="ns1blankspaceRow">');
													aHTML.push('<td style="width:30%;" class="ns1blankspaceRow ns1blankspaceRowShaded">')
														...
														aHTML.push('<tr><td id="ns1blankspaceInvoice_date-' + aReceipts[0].id + '" class="ns1blankspaceSubNote">' +
																	ns1blankspace.util.fd(aReceipts[0]['lineitem.receipt.receiveddate']) + '</td></tr>');

														aHTML.push('<tr><td id="ns1blankspaceInvoice_amount-' + aReceipts[0].id + '" class="ns1blankspaceSubNote">$' +
																	aReceipts[0]['lineitem.receipt.amount'] + '</td></tr>');
													...
													aHTML.push('</td><td class="ns1blankspaceRow" style="padding-left:8px;">');
														...
														$.each(aReceipts, function(j, oReceipt)
														{
															aHTML.push('<tr>');
															...
															aHTML.push('<td style="width:20px;text-align:right;" class="ns1blankspace">' +
																			'<span id="ns1blankspaceInvoice_options_apply-' + oReceipt['id'] + '" class="ns1blankspaceReceiptApply"></span>' +
																			'</td></tr>');
														});
												...
												$('#ns1blankspaceReceiptColumn2Allocate').html(aHTML.join(''));

												$('#ns1blankspaceReceiptColumn2Allocate span.ns1blankspaceReceiptApply').button(
												{
													text: false,
													icons:
													{
												...
												.click(function()
												{
													ns1blankspace.financial.invoice.receipt.allocate.apply.init({xhtmlElementID: this.id});
												})
												.css('width', '15px')
												.css('height', '17px');
											}	
					 				...
					 				apply:
					 				{
						 				init:	function (oParam, oResponse)
						 				{
						 					var iReceiptItemID = ns1blankspace.util.getParam(oParam, 'xhtmlElementID', {index: 1}).value;
						 					...
						 					if (oResponse == undefined)
											{
												...
												oSearch.getResults(function(data)
												{
													ns1blankspace.financial.invoice.receipt.allocate.apply.init(oParam, data)
												});
											}
											else
											{
												ns1blankspace.financial.invoice.receipt.allocate.data.invoiceOutstandingItems = oResponse.data.rows;

												$.each(ns1blankspace.financial.invoice.receipt.allocate.data.invoiceOutstandingItems, function (i, item)
												{
												...
												ns1blankspace.financial.invoice.receipt.allocate.data.invoiceOutstandingAmount = 
																_.sum(_.map(ns1blankspace.financial.invoice.receipt.allocate.data.invoiceOutstandingItems, function (i) {return numeral(i.invoiceoutstandingamount).value()}));

												ns1blankspace.financial.invoice.receipt.allocate.data.receiptItem = $.grep(ns1blankspace.financial.invoice.receipt.allocate.data.receiptItems,
																function (receipt) {return receipt.id == iReceiptItemID});

												if (ns1blankspace.financial.invoice.receipt.allocate.data.receiptItem.length == 1)
												{
						 				...
						 				{
						 					var iReceiptItemID = ns1blankspace.util.getParam(oParam, 'xhtmlElementID', {index: 1}).value;
						 					var iIndexInvoiceOutstandingItem = ns1blankspace.util.getParam(oParam, 'index').value;

						 					if (iIndexInvoiceOutstandingItem < ns1blankspace.financial.invoice.receipt.allocate.data.invoiceOutstandingItems.length &&
															ns1blankspace.financial.invoice.receipt.allocate.data.receiptItemAmountToApply != 0)
											{
												ns1blankspace.financial.invoice.receipt.allocate.data.invoiceItem =
													ns1blankspace.financial.invoice.receipt.allocate.data.invoiceOutstandingItems[iIndexInvoiceOutstandingItem];

												ns1blankspace.financial.invoice.receipt.allocate.data.invoiceItemOutstandingAmount = 
													numeral(ns1blankspace.financial.invoice.receipt.allocate.data.invoiceItem.invoiceoutstandingamount).value();
					...
					edit:		function (oParam, oResponse)
								{
									...
									if (iStep == 1)
									{	
										$('#ns1blankspaceReceiptColumn2Summary').html(ns1blankspace.xhtml.loadingSmall)
									...
									if (iStep == 2)
									{
										...
										$('#ns1blankspaceReceiptColumn2Summary').html(aHTML.join(''));
										...
										//				'<span id="ns1blankspaceReceiptAllocate" class="ns1blankspaceAction">Receipt</span>' +
										//				'</td></tr>');

										aHTML.push('<tr class="ns1blankspace">' +
														'<td class="ns1blankspace" id="ns1blankspaceReceiptAllocateContainer">' +
														'</td></tr>');
										
										aHTML.push('</table>');
													
										$('#ns1blankspaceReceiptColumn2Add').html(aHTML.join(''));

										$('[name="radioBankAccount"][value="' + iDefaultBankAccount + '"]').attr('checked', true);
									
										$('#ns1blankspaceReceiptAmount').val(numeral(numeral(ns1blankspace.objectContextData.amount).value() - numeral(cReceiptedAmount).value()).format('0,0.00'));
										...
										ns1blankspace.financial.invoice.receipt.allocate.init($.extend(true, oParam,
										{	
											receiptedAmount: cReceiptedAmount
										}));
									}
