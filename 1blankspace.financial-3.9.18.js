						...
						if (ns1blankspace.financial.initStatus != 2 || bRefresh)
						{
							if (iStep == -1)
							{
								if (oResponse == undefined)
								{
                                    ...
                                    {
                                        ns1blankspace.financial.initData(oParam, data)
                                    });
                            ...
                            if (iStep == -2)
							{
                                // SET UP BASE CHART OF ACCOUNTS IE 5 ROOT TYPES

                                oParam.onComplete = ns1blankspace.financial.initData;
                                oParam.step = 0;
...
ns1blankspace.financial.home = function (oParam)
					{	
                        ...
                        aHTML.push('<tr class="ns1blankspaceControl">' +
									'<td id="ns1blankspaceControlHealthCheck" class="ns1blankspaceControl" style="padding-bottom:20px;">Health Check</td>' +
									'</tr>');
                        ...
                        aHTML.push('<div id="ns1blankspaceMainHealthCheck" class="ns1blankspaceControlMain"></div>');
                        ...
                        if (ns1blankspace.util.financial.health != undefined)
						{
							$('#ns1blankspaceControlHealthCheck').click(function(event)
							{
								ns1blankspace.show({selector: '#ns1blankspaceMainHealthCheck', refresh: true});
								ns1blankspace.financial.healthCheck.show();
							});
						}	
...
ns1blankspace.financial.debtors =
{
	...
	aHTML.push('<tr><td style="padding-bottom:12px;"><span id="ns1blankspaceFinancialDebtorsRefresh" class="ns1blankspaceAction">' +
						...
						$('#ns1blankspaceFinancialDebtorsRefresh').button(
						{
							label: 'Refresh'
						})
						.click(function()
						...
						.click(function(event)
						{
							...
							[
								...
								'<div style="font-size:2em;">' + ns1blankspace.spaceText + '</div>',
	...
	show:		function (oParam, oResponse)
				{
					...
					{	
						...
						aHTML.push('<tr><td class="ns1blankspaceDate" style="padding-bottom:8px;">' +
										'<input id="ns1blankspaceCreditorsEndDate" data-1blankspace="ignore" class="ns1blankspaceDate ns1blankspaceWatermark" style="padding-left:3px; width:113px; font-size:0.725em; text-align:center;" value="As at">' +
										'</td></tr>');

						aHTML.push('<tr><td style="padding-bottom:12px;"><span id="ns1blankspaceFinancialCreditorsRefresh" class="ns1blankspaceAction">' +
										'Refresh</span></td></tr>');
						...
						$('#ns1blankspaceFinancialCreditorsRefresh').button(
						{
							label: 'Refresh'
						})
						.click(function()
...
ns1blankspace.financial.healthCheck =
{
	show: 	    function (oParam, oResponse)
				{
                    var sStartDate;
					...
					var bShowAccrued = ns1blankspace.util.getParam(oParam, 'showAccrued').value;

                    if (bShowAccrued == undefined)
                    {
                        bShowAccrued = (ns1blankspace.financial.data.settings.accrual == 'Y')
                    }
					...
					if (iStep == 1)	
					{
						...
						aHTML.push('<table class="ns1blankspaceContainer">' +
										'<tr class="ns1blankspaceContainer">' +
										'<td id="ns1blankspaceHealthCheckColumn1" class="ns1blankspaceColumn1Divider" style="width:115px; font-size:0.875em; padding-right:10px;"></td>' +
										'<td id="ns1blankspaceHealthCheckColumn2" style="padding-top:4px; padding-left: 15px;">' +
                                            '<div class="ns1blankspaceSub" style="width:200px;">' +
                                                'Please select the dates and then click the item you want to check and reprocess the GL transactions for.</div>' +
                                        '</td>' +
										'</tr>' +
										'</table>');				
						
						$('#ns1blankspaceMainHealthCheck').html(aHTML.join(''));
						...
						if (ns1blankspace.option.bootstrap)
						{
                            ...
                            aHTML.push('<tr><td class="ns1blankspaceDate">' +
										'<input id="ns1blankspaceHealthCheckStartDate" data-1blankspace="ignore" class="ns1blankspaceDate">' +
										'</td></tr>');
						    ...
						    aHTML.push('<tr>' +
										...
										'</td></tr>' +
										'<tr><td class="ns1blankspaceDate">' +
										'<input id="ns1blankspaceHealthCheckEndDate" data-1blankspace="ignore" class="ns1blankspaceDate">' +
										'</td></tr>');

                            aHTML.push('</table>');

							aHTML.push('<div class="btn-group-vertical" role="group" data-toggle="buttons" id="ns1blankspaceHealthCheckColumnType" style="width:95px; margin-top:3px; text-align:right;">');
						
							if (bShowAccrued)
							{
								aHTML.push('<label class="btn btn-default" id="ns1blankspaceHealthCheckColumnType-2">' +
												'<input type="radio" name="radioType" data-1blankspace="ignore" />Expenses' +
												'</label>');
							}

							aHTML.push('<label class="btn btn-default" id="ns1blankspaceHealthCheckColumnType-3">' +
											'<input type="radio"  name="radioType" data-1blankspace="ignore" />Payments' +
											'</label>');

							if (bShowAccrued)
							{
								aHTML.push('<label class="btn btn-default" id="ns1blankspaceHealthCheckColumnType-5>' +
												'<input type="radio" name="radioType" data-1blankspace="ignore" />Invoices' +
												'</label>');
							}

							aHTML.push('<label class="btn btn-default" id="ns1blankspaceHealthCheckdColumnType-6">' +
											'<input type="radio" name="radioType" data-1blankspace="ignore" />Receipts' +
											'</label>');
							aHTML.push('</div>');

							$('#ns1blankspaceHealthCheckColumn1').html(aHTML.join(''));

                            ns1blankspace.util.initDatePicker({select: 'input.ns1blankspaceDate'});

							$('#ns1blankspaceHealthCheckColumnType label').click(function()
							{
								var aID = (this.id).split('-');
								oParamHealthCheck = 
                                {
                                    object: parseInt(aID[1]),
                                    reprocess: true
                                }

                                if ($('#ns1blankspaceHealthCheckStartDate').val() != '')
                                {
                                    oParamHealthCheck.modifiedDateGreaterThan = $('#ns1blankspaceHealthCheckStartDate').val()
                                }

                                if ($('#ns1blankspaceHealthCheckEndDate').val() != '')
                                {
                                    oParamHealthCheck.modifiedDateLessThan = $('#ns1blankspaceHealthCheckEndDate').val()
                                }

                                ns1blankspace.util.financial.health.init(oParamHealthCheck);
							});
						}
                    }
                }
