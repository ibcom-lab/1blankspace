                                    ...
                                    add: 		function(oParam, oResponse)
                                                    {
                                                        ...
                                                        var aExistingAccounts = ns1blankspace.setup.space.initialise.financials.data.existingAccounts;
                                                        ...
                                                        if (iStep == 1)
                                                        {
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                {
                                                                    ns1blankspace.setup.space.initialise.financials.data.existingAccounts = oResponse.data.rows;
                                                                    oParam.step = 2
                                                        ...
                                                        { 
                                                            ...
                                                            ns1blankspace.status.working('Preparing root accounts... ');

                                                            //Get root from import
                                                            //Get root from existing

                                                            var aExistingAccountsRoot = $.grep(aExistingAccounts, function(account) {return account.parentaccount == ''});
                                                            var iExistingRootAccountID = aExistingAccountsRoot[0].id;
                                                            
                                                            $.each(aImportAccounts, function (a, importAccount)
                                                            {
                                                                if (importAccount.parentaccount == iExistingRootAccountID)
                                                                {
                                                                    importAccount._parentaccount = iImportRootAccountID
                                                                }
                                                            });
                                                            ...
                                                            var aExistingAccountsTypeRoots = $.grep(aExistingAccounts, function(account) {return account.parentaccount == iExistingRootAccountID});
                                                            
                                                            var aImportAccountsToBeProcessed = [];

                                                            $.each(aImportAccounts, function (a, importAccount)
                                                            {
                                                                importAccount._existingAccount = $.grep(aExistingAccounts, function (existingAccount)
                                                                {
                                                                    return (importAccount.title.toLowerCase() == existingAccount.title.toLowerCase()
                                                                        && importAccount.type == existingAccount.type)
                                                                });

                                                                if (importAccount._existingAccount.length == 0)
                                                                {
                                                                    aImportAccountsToBeProcessed.push(importAccount)
                                                                }
                                                            });

                                                            console.log(aImportAccounts)
                                                        ...
                                                        if (iStep == 3)
                                                        {
                                                            // loop through each type looking for children with matching parentaccountext
                                                        ...
                                                        { 
                                                            ...
                                                            if (iAccountIndex < aFinancialAccounts.length)
                                                            {
                                                                ...
                                                                var oAccount = aFinancialAccounts[iAccountIndex];

                                                                var aExistingAccount = $.grep(ns1blankspace.setup.space.initialise.financials.data.existingAccounts, 
                                                                        function (oExistingAccount)
                                                                                    {
                                                                                        return  (   (
                                                                                                        oExistingAccount.title.toLowerCase() == oAccount.title.toLowerCase()
                                                                                                        || oExistingAccount.code.toLowerCase() == oAccount.code.toLowerCase()
                                                                                                    ) 
                                                                                                    && (oExistingAccount.type == oAccount.type)
                                                                                                )
                                                                                    });
    ...
    accounts:
    {
		...
		settings: function (oParam, oResponse)
        {
            if (oResponse == undefined)
            {
                ...
                oSearch.addField('financialaccountcash,financialaccountcashtext,' +
									'financialaccountcreditors,financialaccountcreditorstext,' +
									'financialaccountcurrentearnings,financialaccountcurrentearningstext,' +
									'financialaccountdebtors,financialaccountdebtorstext,' +
									'financialaccountfinancialservicescommission,financialaccountfinancialservicescommissiontext,' +
									'financialaccountproductincome,financialaccountproductincometext,' +
									'financialaccountproductinventory,financialaccountproductinventorytext,' +
									'financialaccountproductpurchases,financialaccountproductpurchasestext,' +
									'financialaccountretainedearnings,financialaccountretainedearningstext,' +
									'financialaccountsalescommission,financialaccountsalescommissiontext,' +
									'financialaccountwriteoff,financialaccountwriteofftext,' +
